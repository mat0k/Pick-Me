...
class PickUpAcceptedService : Service() {
    ...
    private lateinit var localPickUpDbHelper: LocalPickUpDbHelper
    ...
    override fun onCreate() {
        ...
        localPickUpDbHelper = LocalPickUpDbHelper(this)
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val pickUpsListener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                Log.d(
                    "PickUpAcceptedService",
                    "DataSnapshot received: ${snapshot.childrenCount} children."
                )
                val pickups =
                    snapshot.children.mapNotNull { it.getValue(FirebasePickUp::class.java) }
                ...
                justAcceptedPickups.forEach { pickUp ->
                    Log.d(
                        "PickUpAcceptedService",
                        "Processing pickup: ${pickUp.id} accepted by driver: ${pickUp.driverId}"
                    )
                    driversReference.child(pickUp.driverId)
                        .addListenerForSingleValueEvent(object : ValueEventListener {
                            override fun onDataChange(driverSnapshot: DataSnapshot) {
                                ...
                                if (driver != null) {
                                    ...
                                    pickUpAcceptedNotificationService.showNotification(
                                        pickUp,
                                        driver
                                    )
                                    Log.d("PickUpAcceptedService", "Updating driver ID in local database")
                                    localPickUpDbHelper.updateDriverId(pickUp.id, pickUp.driverId)
                                } else {
                                    Log.e(
                                        "PickUpAcceptedService",
                                        "Driver not found for ID: ${pickUp.driverId}"
                                    )
                                }
                            }
                            ...
                            override fun onCancelled(error: DatabaseError) {
                                Log.e(
                                    "PickUpAcceptedService",
                                    "Failed to read driver data: ${error.message}"
                                )
                            }
                        })
                }
